...
- `numeric_order` — global `-n`. First tries `radix::try_numeric_radix` (a stable LSD radix on a parse-once `i64` key) for the common all-integer case; falls back to the `NumericKey` comparison sort the moment any line isn't an exact `i64` (a fractional part, or a magnitude past `i64` range — exponents like `1e3` stay on the radix path as leading-int `1`). The radix output is byte-identical to the comparison path: value order, ties by whole-line bytes (or input order under `-s`/`-u`), and `+` is *not* a sign (GNU treats `+5` as value 0; `split_number` in `compare.rs` matches this). Difftest covers it with pure-integer cases (`scripts/difftest.sh` `gen_ints`).

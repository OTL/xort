...
The external **`-S` (>RAM) merge** sorts each chunk in parallel and is faster
than GNU `sort -S` at the same budget (e.g. ~1.3× on 10M ints).

**Honest caveat.** One path is correctness-complete and byte-compatible with GNU
but **not yet faster** than it, and is being optimized: explicit field-key
sorts (`-k`/`-t`), where re-extracting the key per comparison currently costs us
~1.8×. The numbers in `results.md` reflect that — we'd rather show them than
hide them.

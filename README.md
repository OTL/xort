...
Two paths that started behind GNU are now ahead of it: the external **`-S`
(>RAM) merge** sorts each chunk in parallel (~1.3× faster than `sort -S` at the
same budget on 10M ints), and **single field-key sorts** (`-k`/`-t`) use
decorate-sort-undecorate to extract the key once instead of per comparison
(~1.8× faster than GNU `sort -k` on 10M rows).

**Honest caveat.** *Multi-key* sorts (e.g. `-k2,2 -k3,3n`) still compare keys
per pass and currently trail GNU by ~1.3×; extending DSU to multiple keys is the
next optimization. `results.md` shows this plainly.

...
Two more paths that started behind GNU are now ahead of it: the external
**`-S` (>RAM) merge** sorts each chunk in parallel (~1.3× faster than
`sort -S` at the same budget on 10M ints), and **field-key sorts** (`-k`/`-t`)
use decorate-sort-undecorate to extract each key once instead of per comparison
(~1.4× faster than GNU `sort -k`).

...
same budget on 10M ints), and **field-key sorts** (`-k`/`-t`) use
decorate-sort-undecorate to extract every key once up front instead of on each
comparison — a single key is ~1.8× faster than GNU `sort -k`, and multi-key
sorts (e.g. `-k2,2 -k3,3n`) now edge it out too.
